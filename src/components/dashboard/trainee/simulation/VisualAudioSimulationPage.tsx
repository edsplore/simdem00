...
const VisualAudioSimulationPage: React.FC<VisualAudioSimulationPageProps> = ({
  ...
  useEffect(() => {
    ...
    if (currentItem.type === "message") {
      ...
      if (currentItem.role === "Customer" || currentItem.role === "customer") {
        ...
        setSpeaking(true);
        setMessages(prev => [...prev, { speaker: 'customer', text: currentItem.text || '' }]);
      ...
      } else if (currentItem.role === "Trainee" || currentItem.role === "assistant") {
        setMessages(prev => [...prev, { speaker: 'trainee', text: currentItem.text || '' }]);
      }
    }
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      setMessages([{
        speaker: 'customer',
        text: 'Loading visual-audio simulation...'
      }]);
      ...
      setMessages([{
        speaker: 'customer',
        text: 'Error loading simulation. Please try again.'
      }]);
    } finally {
      setIsStarting(false);
  ...
  return (
    <Box sx={{ height: '100vh', bgcolor: 'white', py: 0, px: 0 }}>
      ...
      {!isCallActive ? (
        ...
        <Box sx={{ display: 'flex', maxWidth: '1200px', mx: 'auto', mt: 2 }}>
              ...
              >
                <Paper
                  ...
                  sx={{
                    ...
                    border: '1px solid',
                    borderLeft: 4,
                    borderColor: currentItem.role === 'customer' ? 'primary.main' : 'success.main',
                    borderRadius: 1,
                    mb: 2,
                  }}
                >
                  ...
                  >
                    <Avatar
                      sx={{
                        width: 32,
                        height: 32,
                        bgcolor: currentItem.role === 'Customer' || currentItem.role === 'customer'
                          ? 'primary.light'
                          : 'success.light',
                      }}
                    >
                  ...
                  <Typography variant="body2">{currentItem.text}</Typography>
                </Paper>

                {/* Trainee recording indicator */}
                {(currentItem.role === "Trainee" || currentItem.role === "assistant") && isRecording && (
                  <Paper
                    sx={{
                      p: 2,
                      bgcolor: "grey.100",
                ...
                {(currentItem.role === "Customer" || currentItem.role === "customer") && speaking && (
                  <Paper
                    sx={{
                      p: 2,
                      bgcolor: "grey.100",
