      ...
      rec.onstop = async () => {
        try {
          ...
          if (simulationProgressId && blob.size > 0) {
            console.log('Uploading audio, size:', blob.size);
          ...
          resolve(''); // Don't fail, let simulation complete
        } finally {
          convoChunksRef.current = [];
        }
      };
  ...
  const handleEndCall = async () => {
    ...
    try {
      ...
      if (audioUrl) {
        ...
        // Add small delay to ensure backend processes the upload
